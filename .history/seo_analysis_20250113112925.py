...
# Ensure we have 'punkt_tab' and possibly other data:
...
###############################################################################
# PAGE SPEED INSIGHTS (two strategies: mobile + desktop)
###############################################################################
def check_page_speed_insights(url, api_key=None, strategy="mobile"):
    if not api_key:
        logging.info(f"No API key -> skipping PageSpeed for {url}, strategy={strategy}")
        return {"performance_score": None, "error": "No API key"}

    endpoint = "https://www.googleapis.com/pagespeedonline/v5/runPagespeed"
    params = {"url": url, "key": api_key, "strategy": strategy}
    logging.info(f"PageSpeed call for {url}, strategy={strategy}")
    try:
...
def attempt_password_login_if_needed(driver, base_url, site_password):
    ...
    if not site_password:
        logging.info("No password provided -> skip login.")
        return

    try:
        # 1) Load the password-protected site, fill the form if found
        driver.get(base_url)
        time.sleep(2)
        logging.info("Loaded base URL. Searching for WP password form...")

        for attempt in range(2):
            logging.info(f"Password login attempt #{attempt+1}")
            ...
            if forms:
                ...
                logging.info("No forms found. Trying fallback outside form.")
                try:
                    pass_input = driver.find_element(By.NAME, "password_protected_pwd")
                    logging.info("Found 'password_protected_pwd' fallback.")
                except:
                    logging.info("Fallback also failed -> no pass input found.")
            ...
            if pass_input:
                ...
                try:
                    ...
                    logging.info("Found wp-submit button -> clicking.")
                    submit_btn.click()
                except:
                ...
                time.sleep(2)
                # 2) re-get base url so we see the logged-in state
                logging.info("Re-get base URL to confirm login success.")
                driver.get(base_url)
                time.sleep(2)
            else:
                logging.info("No password input discovered -> skipping further attempts.")
                break

        # 3) After login, forcibly load <original_domain>/sitemap.xml
        # E.g. if base_url is https://original_domain.com
        # we do driver.get(base_url.rstrip('/') + '/sitemap.xml')
        # to ensure we see the correct sitemap from the primary domain.
        primary_sitemap = base_url.rstrip("/") + "/sitemap.xml"
        logging.info(f"After login -> forcing driver.get({primary_sitemap}) to ensure main sitemap.")
        driver.get(primary_sitemap)
        time.sleep(2)

    except Exception as e:
        logging.exception(f"Exception in password login logic: {e}")

###############################################################################
# SITEMAP (Now that we do a direct load after login)
###############################################################################
...
def gather_links_from_sitemap(base_url, max_pages, status_callback=None, site_password=""):
    ...
    # Now attempt to load raw sitemap.xml with requests (the site may or may not allow it).
    sitemap_url = base_url.rstrip("/") + "/sitemap.xml"
    if status_callback:
        status_callback(f"Attempting to fetch {sitemap_url}")
    logging.info(f"Fetching {sitemap_url} via requests with normal redirects.")
    try:
        resp = requests.get(sitemap_url, timeout=10, allow_redirects=True)
        resp.raise_for_status()
    except Exception as e:
        logging.warning(f"Error fetching {sitemap_url}: {e}")
        # fallback -> maybe /sitemap_index.xml
        ...
        try:
            ...
            driver.quit()
            raise Exception(f"Both /sitemap.xml and /sitemap_index.xml failed: {e2}")
...
def scroll_and_collect(drv):
    """Scroll to bottom a few times to capture JS-lazyloaded links."""
    for _ in range(3):
...
def selenium_bfs_concurrent(base_url, max_pages, status_callback=None,
    ...
    logging.info("Starting BFS concurrency...")

    # We'll do password login first
    ...
    logging.info(f"BFS concurrency -> {n_workers} workers from {cores} cores.")
    ...
    def bfs_worker(drv):
        while True:
            ...
            logging.info(f"BFS Worker visiting {url_}")
            try:
                drv.get(url_)
...
def analyze_page(driver, url, status_callback, current_idx, total_count,
    ...
    logging.info(f"Analyzing page: {url} (idx={current_idx}/{total_count})")
    data = {
        "URL": url,
        "Title": "",
        "TitleLength": 0,
    ...
    try:
        ...
        data["Error"] = str(e)
        logging.exception(f"Exception analyzing page {url}: {e}")
...
def analyze_pages_in_pool(urls, driver_path, status_callback, progress_callback,
    ...
    cores = multiprocessing.cpu_count()
    n_workers = max(int(0.75 * cores), 1)
    logging.info(f"analyze_pages_in_pool -> {n_workers} workers from {cores} cores.")
    ...
    if not urls:
        logging.info("No URLs to analyze, returning empty.")
        return []
    ...
    with ThreadPoolExecutor(max_workers=n_workers) as executor:
        ...
        for fut in as_completed(future_map):
            try:
                ...
                logging.exception(f"Thread error in analyze_pages_in_pool: {e}")
...
class ScraperWorker(QObject):
    ...
    def run(self):
        try:
            ...
            logging.info(f"Worker run => base_url={base_url}")
            ...
            if not links:
                logging.info("No links from sitemap => fallback BFS.")
            ...
            # 2) Analyze
            logging.info("Now analyzing pages in concurrency pool.")
...
class MainWindow(QMainWindow):
    ...
    def start_scraping(self):
        if self.enable_logging_check.isChecked():
            # Create a log file in the chosen directory
            ...
            logging.info("Detailed logging to file enabled.")
        else:
            logging.basicConfig(
        ...
        logging.info("start_scraping invoked from GUI.")
        ...
        logging.info(f"Domain={domain}, MaxPages={max_pages}, DriverPath={driver_path}, "
                     f"PasswordLen={len(site_password)}, PageSpeedKeyLen={len(pagespeed_api)}")
...
if __name__ == "__main__":
    logging.info("Launching BeykeTechnik SEO Analyzer.")
