...
# YOUR PAGESPEED INSIGHTS API KEY (Use environment variable or secret manager in production!)
PAGESPEED_API_KEY = "AIzaSyB8R9HLyxA6cvv2PLzhh4fWXxlXlSopnpg"
...
def check_page_speed_insights(url, api_key=None, strategy="mobile"):
    """
    Calls Google's PageSpeed Insights API and returns a small dictionary:
        {
          "performance_score": (0..100 or None),
          "mobile_friendliness": "PASS"/"FAIL"/"N/A"...
          "error": "..." if any
        }
    Docs: https://developers.google.com/speed/docs/insights/overview

    This function can be slow and consume quota if you do it for every page.
    """
    if not api_key:
    ...
    try:
        ...
        # Lighthouse performance is: data["lighthouseResult"]["categories"]["performance"]["score"]
        # typically a float 0.0..1.0
        # Convert to 0..100
        perf = None
        try:
            perf = data["lighthouseResult"]["categories"]["performance"]["score"]
            perf = int(perf * 100)
        except:
            perf = None

        # For mobile_friendliness, we can do a simple check:
        # data["loadingExperience"]["overall_category"] might exist.
        # If the strategy is 'mobile' and the "overall_category" is "FAST"/"AVERAGE"/"SLOW"
        # we can interpret it. This is somewhat deprecated, but we'll try:
        mobile_friendly = "N/A"
        try:
            le = data.get("loadingExperience", {})
            oc = le.get("overall_category")
            if oc:
                # possible values: FAST, AVERAGE, SLOW
                if oc == "FAST":
...
def selenium_bfs_concurrent(base_url, max_pages, status_callback, driver_count=3, driver_path=None):
    ...
    - We do skip file extensions in IGNORE.
    """
    if status_callback:
...
def analyze_on_page_seo(html, url, pagespeed_data=None):
    ...
    Integrate pagespeed_data (performance_score, mobile_friendliness) if available.
    """
    ...
    # 10) PageSpeed data
    if pagespeed_data:
        results["PerformanceScore"] = pagespeed_data.get("performance_score", None)
        results["MobileFriendlinessCheck"] = pagespeed_data.get("mobile_friendliness", "N/A")
        results["PageSpeedError"] = pagespeed_data.get("error", None)
    else:
        results["PerformanceScore"] = None
        results["MobileFriendlinessCheck"] = "N/A"
        results["PageSpeedError"] = None
...
def compute_score_and_recommendations(data):
    ...
    if isinstance(perf_score, int):
        ...
        recs.append("Consider measuring site performance with PageSpeed insights.")
...
def analyze_page(driver, url, status_callback, current_idx, total_count, api_key=None):
    """
    Load a page with Selenium, fetch PageSpeed data, then do on-page SEO analysis & scoring.
    """
    try:
        driver.get(url)
        ...
        # Call PageSpeed if we have an API key
        pagespeed_info = None
        if api_key:
...
class ScraperWorker(QObject):
    ...
    3) Analyze pages with SEO checks & PageSpeed API (via provided key).
    4) Provide a progress update for analysis.
    5) Output CSV & HTML.
    """
    ...
    def __init__(self, domain, max_pages, driver_path, output_dir):
        ...
        # Use the global PAGESPEED_API_KEY if you want
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("On-Page SEO Analyzer with PageSpeed Insights")
